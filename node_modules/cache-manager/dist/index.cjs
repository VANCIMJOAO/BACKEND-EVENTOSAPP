...
var createCache = (options) => {
  ...
  const wrap = async (key, fnc, ttl2, refreshThreshold) => coalesceAsync(key, async () => {
    let value;
    let i = 0;
    let remainingTtl;
    ...
    if (value === void 0) {
      ...
      await set(stores, key, result, runIfFn(ttl2, result) ?? options?.ttl);
      return result;
    }
    const ms = runIfFn(ttl2, value) ?? options?.ttl;
